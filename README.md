...
- `api/live-updates.js` — función serverless que mantiene el marcador EN VIVO de los partidos actualizado (marcador, goles y status) consultando API-Football cada 5 minutos (ver sección "Marcador en vivo" más abajo)
...
- `.github/workflows/live-updates.yml` — workflow de GitHub Actions que llama a `api/live-updates` cada 5 minutos (ver sección "Marcador en vivo")
...
4. Antes de darle Deploy, ve a **Environment Variables** y agrega:
   ...
   - **Name:** `CRON_SECRET` (opcional pero recomendado) — **Value:** cualquier texto aleatorio largo; Vercel lo usa automáticamente para que solo el Cron Job pueda ejecutar `api/actualizar-resultados`, y también protege `api/live-updates` (mismo valor, ver sección "Marcador en vivo")
...
- **Predicciones** (`profetas/matches/matches/{matchId}/predictions/{profileId}`): la propia siempre se puede leer; las de los demás solo una vez que su campo `visible` quede en `true` (lo pone `api/live-updates.js`/`api/actualizar-resultados.js` apenas pasa el kickoff del partido — nunca el navegador). Solo se puede crear/editar la predicción propia (`ownerUid` == el propio `uid`), o un admin (necesario para las predicciones automáticas del bot). **Importante:** si actualizaste desde una versión anterior de estas reglas, tienes que volver a pegar el archivo completo — la versión vieja usaba una función que hacía que la app no pudiera cargar las predicciones de nadie (ver nota más abajo, sección "Marcador en vivo").
...
Cada partido tiene un campo `status`: `scheduled` (por jugar) → `live` (en curso, marcador provisional) → `finished` (terminado, resultado oficial). El paso de `scheduled` a `live` es automático cuando llega la hora de kickoff guardada; mientras un partido está `live`, la app calcula los puntos de todos con el marcador actual (dejándolo claro con el ícono 🔴 EN VIVO) y se actualiza sola en pantalla vía `onSnapshot`, sin que nadie tenga que recargar.

Quién actualiza el marcador mientras el partido está en curso:
- **Automático**: `api/live-updates.js` consulta API-Football (marcador y, si el plan lo permite, la lista de goles con equipo+minuto) y escribe los cambios directamente en Firestore con el Admin SDK — no depende de que alguien tenga la app abierta. Antes de gastar cuota, revisa `/status` (que no cuenta contra el límite diario) y se salta la ejecución si quedan menos de 15 peticiones ese día.
- **Manual (respaldo)**: en Gestionar → "🔴 Partidos en vivo" se puede editar el marcador directamente o registrar un gol uno por uno (equipo + minuto), exactamente igual que ya existía para partidos finalizados. Esto nunca depende de que la API responda a tiempo. También hay un botón "Marcar en vivo ahora" para forzar el paso de `scheduled` a `live` sin esperar al cron. Un partido se puede finalizar a mano en cualquier momento con "Finalizar partido", que congela el marcador actual como resultado oficial.

**Importante sobre la frecuencia de 5 minutos**: el plan gratis de Vercel (Hobby) limita los Cron Jobs a **una sola ejecución al día**, sin importar la expresión cron que se use — por eso `vercel.json` no incluye una entrada para `api/live-updates` (agregarla ahí no lograría nada en Hobby). En su lugar, `.github/workflows/live-updates.yml` (GitHub Actions, gratis, no depende del plan de Vercel) llama al endpoint cada 5 minutos. Para que funcione:

1. En GitHub: **Settings → Secrets and variables → Actions → New repository secret**, nombre `CRON_SECRET`, mismo valor que configuraste en Vercel.
2. Listo — el workflow ya está en el repo y arranca solo. GitHub no garantiza el minuto exacto (puede atrasarse unos minutos en horas pico), pero es suficiente para un marcador que se refresca cada 5 minutos.
3. Si en algún momento pasas a Vercel Pro (que sí permite cron cada minuto), puedes agregar `{ "path": "/api/live-updates", "schedule": "*/5 * * * *" }` a `vercel.json` y desactivar el workflow de GitHub — el endpoint funciona igual desde cualquiera de los dos.

Si tu plan de API-Football no da acceso al endpoint de eventos (`/fixtures/events`), la lista de goles minuto a minuto simplemente no se llena — el marcador total sigue actualizándose sin problema, `api/live-updates.js` lo maneja con un intento aparte que nunca bloquea el marcador.

**Corrección importante encontrada de paso**: mientras se probaba esta función se descubrió que las predicciones de los demás en realidad **no estaban cargando para nadie** (ni admin ni jugadores) desde que se implementó "predicciones ajenas ocultas también a nivel de datos" — no es un problema nuevo de esta función, sino de esa anterior. Firestore rechaza una consulta completa (no la filtra documento por documento) si esa consulta, sin restricciones, pudiera llegar a devolver algún documento que la regla no deja leer; como la app pedía "todas las predicciones" sin ningún filtro, y siempre hay algún partido por arrancar con predicciones ajenas de por medio, la consulta entera fallaba en silencio (quedaba registrado como error en la consola del navegador, pero la interfaz simplemente mostraba "nadie ha predicho" en vez de un mensaje de error).

La solución (ya incluida en este mismo cambio) fue agregarle a cada predicción un campo `visible` que las funciones serverless activan cuando corresponde, y hacer que la app pida solo `where('visible','==',true)` (predicciones ya reveladas) o `where('ownerUid','==', tu uid)` (las tuyas, siempre) — nunca "todas". Firestore sí puede validar por adelantado que esas dos consultas son seguras. Dos cosas a tener en cuenta después de desplegar este cambio:

1. **Hay que volver a pegar `firestore.rules` en Firebase Console** (paso 3.2) — la regla de predicciones cambió.
2. **Las predicciones de partidos que ya pasaron su kickoff antes de este cambio no tienen el campo `visible` todavía** — se les pone solo, pero recién la próxima vez que corra `api/live-updates` o `api/actualizar-resultados`. Para no esperar hasta 12 horas (la próxima corrida del cron de resultados), dispara cualquiera de los dos a mano una vez después de desplegar:
   ```
   curl https://tu-app.vercel.app/api/live-updates -H "Authorization: Bearer TU_CRON_SECRET"
   ```
...
- **Predicciones en subcolección por partido**: en vez de vivir todas mezcladas en un solo documento, cada predicción es su propio documento en `profetas/matches/matches/{matchId}/predictions/{profileId}`. Esto es lo que permite que las reglas de Firestore verifiquen, por documento, que cada quien solo pueda crear/editar su propia predicción — algo imposible si todas vivieran juntas en un array o mapa gigante.
- **Predicciones ajenas ocultas también a nivel de datos (campo `visible`)**: no solo la interfaz oculta las predicciones ajenas hasta el kickoff — las reglas de Firestore también lo hacen, comparando el campo `visible` de cada predicción (`resource.data.visible == true`). Ese campo lo pone `api/live-updates.js`/`api/actualizar-resultados.js` (Admin SDK, nunca el navegador) apenas pasa el kickoff del partido — solo se puede leer la predicción de alguien más una vez está en `true`, o siempre si es la tuya propia (por `profileId` o por `resource.data.ownerUid`) o eres admin. La primera versión de esto usaba una función `matchKickoffPassed(matchId)` que comparaba la hora directamente en la regla; se cambió a un campo `visible` explícito porque Firestore rechaza una consulta *completa* (no la filtra por documento) si, sin restricciones, pudiera devolver algún documento que la regla no deja leer — con `matchKickoffPassed` eso hacía que la consulta de "todas las predicciones" fallara siempre que hubiera al menos un partido por arrancar, es decir, casi siempre. Por eso `loadAll()` en `app.js` nunca pide "todas las predicciones" a secas: pide `where('visible','==',true)` (ya reveladas) o `where('ownerUid','==', tu uid)` (las tuyas) combinadas con `or()`, ambas contra `collectionGroup(db,'predictions')`.
- **La regla de lectura de `predictions` vive en un bloque `{path=**}` aparte, no anidada bajo `matches/{matchId}`**: dos requisitos, los dos necesarios y ninguno obvio, para que esas consultas con filtro funcionen. (1) La condición tiene que estar escrita como comparación directa de campo (`resource.data.campo == valor`), nunca envuelta en `.get(campo, default)` — con `.get()` de por medio Firestore ya no logra probar que la consulta es segura y la rechaza entera, aunque el dato en sí sea correcto. (2) Una regla de lectura *anidada* dentro de la ruta exacta del padre (`match /profetas/matches/matches/{matchId} { match /predictions/{profileId} { allow read: ... } } }`) nunca se usa para validar una **collectionGroup query** — solo sirve para leer un documento puntual por su ruta completa. Para que Firestore la reconozca en una consulta de colección-grupo hay que declararla en un bloque separado con el comodín recursivo: `match /{path=**}/predictions/{profileId} { allow read: ...; }`. Nos costó tres vueltas de "actualiza las reglas y prueba de nuevo" descubrir esto — quedó documentado en `firestore.rules` para no repetirlo.
- **`kickoff` en milisegundos, no en texto**: los partidos nuevos guardan la hora de inicio como número (epoch) en vez del texto que entrega el campo `datetime-local`, para que las reglas de Firestore puedan comparar fechas. `bogotaDateStr()` (en `app.js` y en `api/actualizar-resultados.js`) calcula la fecha "de Colombia" (UTC-5 fijo, sin horario de verano) a partir de esos milisegundos sin depender de la zona horaria de quien ejecute el código — importante para no pedirle a API-Football la fecha equivocada en partidos nocturnos.
...
- **Escape de HTML (protección contra XSS almacenado)**: toda la interfaz se arma pegando texto en `innerHTML` (no hay un framework con auto-escape), así que cualquier texto que alguien más controla — su nombre de perfil, el nombre del goleador que escribió, el color o código de un equipo, la URL de una foto/escudo — pasa por `escapeHtml()` (en `app.js`) antes de meterse en el HTML. Sin esto, alguien podía poner como "nombre" algo como `<img src=x onerror="...código...">` y ese código se ejecutaría en el navegador de cualquiera que viera ese nombre, incluido un administrador — dándole control de esa sesión (y, con ella, de cualquier predicción). Las reglas de Firestore no protegen contra esto: permiten guardar cualquier texto en un campo propio, es la interfaz la que tiene que tratarlo siempre como texto y no como HTML.
- **Marcador en vivo (`status: scheduled|live|finished`)**: `matchStatus(m)` (en `app.js` y duplicada en `api/live-updates.js`) es la única fuente de verdad sobre en qué etapa va un partido; si un partido viejo no tiene el campo `status` (datos de antes de esta función), se infiere solo: con marcador ya es `finished`, sin marcador es `scheduled` — no hizo falta migrar nada. `api/live-updates.js` pasa los partidos de `scheduled` a `live` por tiempo (sin gastar cuota de API) apenas llega el kickoff, y mientras estén `live` les actualiza marcador, status y goles consultando API-Football. `computeStandings()` no distingue `live` de `finished` — a ambos los cuenta con el marcador actual, que es justo lo que hace que los puntos "en vivo" ya aparezcan sumados en la Tabla; el ícono 🔴 EN VIVO (banner en Tabla, insignia en la tarjeta del partido y en los modales de predicciones) es solo para dejar claro que ese marcador todavía puede cambiar.
- **Goles (equipo + minuto)**: cada partido puede tener un array `goals` (`[{team:'home'|'away', minute}]`), llenado por `api/live-updates.js` vía `/fixtures/events` de API-Football (mejor esfuerzo — si el plan contratado no da acceso a ese endpoint, o falla puntualmente, sencillamente no se actualiza y el marcador total sigue funcionando igual) o registrado a mano en Gestionar → "🔴 Partidos en vivo". Registrar un gol a mano también incrementa el marcador correspondiente; quitarlo lo decrementa. `goalsListHtml()` en `app.js` es la única función que dibuja esa lista, reutilizada tanto en las tarjetas de partido como en el modal "Ver predicciones".
- **Respaldo manual del marcador en vivo**: en Gestionar, cualquier partido `live` se puede editar directamente (marcador o gol por gol) sin depender de que `api/live-updates.js` haya corrido a tiempo o de que API-Football responda — es la misma idea que ya existía para partidos finalizados, extendida a partidos en curso. También hay "Marcar en vivo ahora" (fuerza el paso de `scheduled` a `live` sin esperar al cron) y "Finalizar partido" (congela el marcador actual como resultado oficial en cualquier momento, sin depender de que la API reporte el final).
- **Competencia (`competition: fpc|mundial`)**: cada equipo y cada partido tiene este campo (por defecto `fpc` si no está — datos de antes de esta función). El Mundial 2026 es para probar que el rastreo automático funciona con otra liga de API-Football (`league=1&season=2026`, contra `league=239` de la FPC) sin mezclarse con la competencia real: `computeStandings()`, el selector de campeón de Pre-temporada y "Liga real" solo cuentan/muestran equipos y partidos `fpc`. En Gestionar, el formulario de "Agregar partido" filtra los equipos de Local/Visitante según la competencia elegida (`teamOptions(competition)`), y "Borrar datos de prueba del Mundial" limpia de una vez los partidos (con sus predicciones) y equipos `mundial` cuando ya no se necesiten. `api/live-updates.js` agrupa los partidos en vivo por competencia + fecha (no solo fecha) porque un mismo día puede tener partidos de ambas, y cada grupo usa la liga que le corresponde — comparten el mismo control de cuota diaria, no hay uno aparte por competencia.
