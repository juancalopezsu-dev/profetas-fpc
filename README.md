...
- **Predicciones ajenas ocultas también a nivel de datos**: no solo la interfaz oculta las predicciones ajenas hasta el kickoff — las reglas de Firestore también lo hacen. `matchKickoffPassed(matchId)` compara el `kickoff` del partido (guardado en milisegundos, no en texto, para poder comparar fechas de forma confiable) contra la hora del servidor; solo se puede leer la predicción de alguien más si el partido ya cerró, eres admin, o es la tuya propia (siempre visible, para que no "desaparezca" de tu pantalla con la sincronización en tiempo real). Antes esto era solo una convención de la interfaz — alguien con conocimientos técnicos podía leer las predicciones ajenas directamente desde la consola del navegador; ahora Firestore mismo lo rechaza.
