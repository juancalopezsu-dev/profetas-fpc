...
- **Predicciones ajenas ocultas también a nivel de datos (campo `visible`)**: no solo la interfaz oculta las predicciones ajenas hasta el kickoff — las reglas de Firestore también lo hacen, comparando el campo `visible` de cada predicción (`resource.data.visible == true`). Ese campo lo pone `api/live-updates.js`/`api/actualizar-resultados.js` (Admin SDK, nunca el navegador) apenas pasa el kickoff del partido — solo se puede leer la predicción de alguien más una vez está en `true`, o siempre si es la tuya propia (por `profileId` o por `resource.data.ownerUid`) o eres admin. La primera versión de esto usaba una función `matchKickoffPassed(matchId)` que comparaba la hora directamente en la regla; se cambió a un campo `visible` explícito porque Firestore rechaza una consulta *completa* (no la filtra por documento) si, sin restricciones, pudiera devolver algún documento que la regla no deja leer — con `matchKickoffPassed` eso hacía que la consulta de "todas las predicciones" fallara siempre que hubiera al menos un partido por arrancar, es decir, casi siempre. Por eso `loadAll()` en `app.js` nunca pide "todas las predicciones" a secas: pide `where('visible','==',true)` (ya reveladas) o `where('ownerUid','==', tu uid)` (las tuyas) combinadas con `or()`. **Importante para que esto funcione**: la condición de la regla tiene que estar escrita EXACTAMENTE como el filtro de la consulta — mismo campo, comparación directa (`resource.data.campo == valor`), sin envolverla en `.get(campo, default)` ni nada parecido. La primera versión de esta regla usaba `resource.data.get('visible', false) == true` (para no arriesgarse a un error si el campo no existía) y esa forma, aunque semánticamente equivalente, hizo que Firestore ya no pudiera probar que la consulta era segura y rechazara la lectura de predicciones para *todo el mundo* — el error salía silencioso en la consola del navegador, así que la interfaz simplemente mostraba "No predijo" y 0 puntos en vez de un error visible. Se corrigió a comparación directa, que es el único patrón que Firestore reconoce como seguro para consultas con filtro.
