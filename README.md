...
- **Predicciones ajenas ocultas también a nivel de datos (campo `visible`)**: no solo la interfaz oculta las predicciones ajenas hasta el kickoff — las reglas de Firestore también lo hacen, comparando el campo `visible` de cada predicción (`resource.data.get('visible', false) == true`). Ese campo lo pone `api/live-updates.js`/`api/actualizar-resultados.js` (Admin SDK, nunca el navegador) apenas pasa el kickoff del partido — solo se puede leer la predicción de alguien más una vez está en `true`, o siempre si es la tuya propia o eres admin. La primera versión de esto usaba una función `matchKickoffPassed(matchId)` que comparaba la hora directamente en la regla; se cambió a un campo `visible` explícito porque Firestore rechaza una consulta *completa* (no la filtra por documento) si, sin restricciones, pudiera devolver algún documento que la regla no deja leer — con `matchKickoffPassed` eso hacía que la consulta de "todas las predicciones" fallara siempre que hubiera al menos un partido por arrancar, es decir, casi siempre. Por eso `loadAll()` en `app.js` nunca pide "todas las predicciones" a secas: pide `where('visible','==',true)` (ya reveladas) o `where('ownerUid','==', tu uid)` (las tuyas) combinadas con `or()` — ambos filtros calzan con las condiciones de la regla, que es lo que le permite a Firestore comprobar de antemano que la consulta es segura.
